...
class TopBalanceTransaction(object):
	...
	def process(self):
		...
		select_stmt = self.session.prepare("SELECT c_w_id, c_d_id, c_first, c_middle, c_last, c_balance FROM top_balance LIMIT 10");
		result_count = self.session.execute(select_stmt)
		...
		for row in result_list:
			select_w_name = self.session.prepare("SELECT w_name FROM warehouse WHERE w_id = ?");
			select_d_name = self.session.prepare("SELECT d_name FROM district WHERE d_w_id = ? AND d_id = ?");

			result_w_name = self.session.execute(select_w_name, [row.c_w_id])
			result_d_name = self.session.execute(select_d_name, [row.c_w_id, row.c_d_id])
