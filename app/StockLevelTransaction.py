...
class StockLevelTransaction(object):
	...
	def __init__(self, session, consistencyLevel, w_id, d_id, stockThreshold, numOfLastOrder):
	...
	def initPreparedStmts(self):
		...
		if self.consistencyLevel == '1' :
			...
			self.select_quantity.consistency_level = ConsistencyLevel.ONE
		else:
			self.select_d_next_o_id.consistency_level = ConsistencyLevel.QUORUM
			self.select_last_l_order.consistency_level = ConsistencyLevel.QUORUM
			self.select_quantity.consistency_level = ConsistencyLevel.QUORUM
